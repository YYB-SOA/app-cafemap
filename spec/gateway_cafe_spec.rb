...
describe 'Tests Cafe Nomad API library' do
  ...
  describe 'Cafe Nomad Store Content' do
    ...
    it 'HAPPY: should pass corect name' do
      _(@cafe_info.map(&:name)).must_equal ans_sheet('name', @yaml_keys,CAFE_CORRECT)

    end

    it 'HAPPY: should pass corect city' do
      _(@cafe_info.map(&:city)).must_equal ans_sheet('city', @yaml_keys,CAFE_CORRECT)
    end

    it 'HAPPY: should pass corect url' do
      _(@cafe_info.map(&:url)).must_equal ans_sheet('url', @yaml_keys,CAFE_CORRECT)
    end

    it 'HAPPY: should pass corect address' do
      _(@cafe_info.map(&:address)).must_equal ans_sheet('address', @yaml_keys,CAFE_CORRECT)
    end

    it 'HAPPY: should pass corect latitude' do
      _(@cafe_info.map(&:latitude)).must_equal ans_sheet('latitude', @yaml_keys,CAFE_CORRECT)
    end

    it 'HAPPY: should pass corect longitude' do
      _(@cafe_info.map(&:longitude)).must_equal ans_sheet('longitude', @yaml_keys,CAFE_CORRECT)
    end

    it 'HAPPY: should pass corect limited_time' do
      _(@cafe_info.map(&:limited_time)).must_equal ans_sheet('limited_time', @yaml_keys,CAFE_CORRECT)
    end

    it 'HAPPY: should pass corrected  mrt' do
      _(@cafe_info.map(&:mrt)).must_equal ans_sheet('mrt', @yaml_keys,CAFE_CORRECT)
    end

    it 'HAPPY: should pass corrected  open_time' do
      _(@cafe_info.map(&:open_time)).must_equal ans_sheet('open_time', @yaml_keys,CAFE_CORRECT)
    end

    it 'HAPPY: should pass identitied standing_desk' do
      _(@cafe_info.map(&:standing_desk)).must_equal ans_sheet('standing_desk', @yaml_keys,CAFE_CORRECT)
    end

    it 'HAPPY: should pass identitied socket' do
      _(@cafe_info.map(&:socket)).must_equal ans_sheet('socket', @yaml_keys,CAFE_CORRECT)
    end
